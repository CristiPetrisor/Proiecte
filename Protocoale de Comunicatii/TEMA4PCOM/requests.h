...
char *recv_post_request(int socket, char host[16], char *command, char *user[1], char *token);

// Functia pentru primirea raspunsului de la server pentru comanda de GET
char *recv_get_delete_request(int socket, char host[16], char *command, char *token, char *cookies[1], char *get_delete);
